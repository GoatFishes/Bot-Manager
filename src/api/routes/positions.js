...
module.exports = async () => {
    ...
    app.use(route.get('/', async (ctx) => {
        try {
            ...
            utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Determine position type and fetching all orders from the database`)
            let orders
            if (type === "paperTrade") {
                orders = await db.selectPaperOrdersByStatus(["Filled"])
            } else {
                orders = await db.selectOrdersByStatus(["Filled"])
            }

            // making the call to calcualte here
            for (let i = 0; i < orders.length; i += 1) {
                utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Processing orders into positions: ${i + 1}/${orders.length}`)
            ...
            utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Determine position type and build position response`)
            if (type === "paperTrade") {
                positions = await db.selectPaperPositions()
            } else {
                positions = await db.selectPositions()
            }
        ...
        } catch (e) { throw new utils.ExceptionHandler(constants.RESPONSE_CODES.APPLICATION_ERROR, `Fatal error on Positions building : ${e}`) }
    }))

    return app
}

// TODO: get rid of this code in favour of something cleaner ideally as part of the utils node package
const ordersRecursion = async (params) => {
    ...
    if (contractZeroCounter === 0) {
        ...
        utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Adding position with id: ${positionId} to the databse`)
        if (type === "paperTrade") {
            await db.insertPaperPosition([positionId, botId, entryPrice, initialMargin, startTime, endTime, side, size, pnl, roe, averageLeverage, averagePrice])
        } else {
            await db.insertPosition([positionId, botId, entryPrice, initialMargin, startTime, endTime, side, size, pnl, roe, averageLeverage, averagePrice])

        }

        for (let i = 0; i < orderIds.length; i += 1) {
            utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Updating the orderId for all related orders`)
            if (type === "paperTrade") {
                await db.updatePaperOrderPositionId([positionId, orderIds[i]])

            } else {
                await db.updateOrderPositionId([positionId, orderIds[i]])
            }
        }

        orderIds.shift()

        utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Reseting persistance variables`)
